...
/* ─── GHOSTING THE NATIVE HEADER & MANAGE APP BUTTONS ─── */
header[data-testid="stHeader"] { 
    background: transparent !important; box-shadow: none !important; pointer-events: none !important;
}

/* Aggressively hide ALL Streamlit cloud clutter, logos, and badges */
...
/* Specifically kill the Streamlit Logo and Manage App links */
a[href*="streamlit.io"], a[title="Manage app"], .viewerBadge_container {
    display: none !important; opacity: 0 !important; visibility: hidden !important; pointer-events: none !important; position: absolute !important; width: 0 !important; height: 0 !important;
}

/* Permanently hide native sidebar toggles */
...
.premium-header {
    ...
    backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px); border-bottom: 1px solid var(--border-glow); border-radius: 16px;
...
/* ─── GLASSMORPHISM METRIC CARDS (PERFECT CENTERING FIX) ─── */
.glass-card, .plat-card {
    ...
    border-radius: 16px; padding: 24px; transition: all 0.3s ease; box-shadow: 0 4px 24px -1px rgba(0, 0, 0, 0.2);
    /* Forcing internal alignment to absolute center */
...
.glass-card:hover, .plat-card:hover { transform: translateY(-5px); border-color: var(--border-glow); box-shadow: 0 8px 32px 0 rgba(0, 240, 255, 0.15); }
...
.kpi-label { font-size: 0.8rem; color: var(--text-muted); text-transform: uppercase; letter-spacing: 1px; margin-top: 8px; font-weight: 600; text-align: center !important; width: 100%; }
...
@media (max-width: 768px) {
    .block-container { padding: 1rem !important; padding-bottom: 90px !important; }
    
    /* Perfect Card Centering for Mobile */
    [data-testid="stHorizontalBlock"] { flex-direction: column !important; gap: 15px !important; }
    [data-testid="column"] { width: 100% !important; flex: 1 1 100% !important; min-width: 100% !important; display: flex !important; flex-direction: column !important; align-items: center !important; justify-content: center !important; }
    .glass-card, .plat-card { width: 100% !important; margin: 0 auto !important; max-width: 350px !important; }
    ...
    /* Kill Mobile Fullscreen UI traps */
    [data-testid="StyledFullScreenButton"], button[title="View fullscreen"] { display: none !important; opacity: 0 !important; pointer-events: none !important; }

    /* Force mobile browsers to scroll */
    [data-testid="stPlotlyChart"], .stPlotlyChart, .js-plotly-plot, .plotly { touch-action: pan-y !important; width: 100% !important; padding-bottom: 15px !important; }
    [data-testid="stDataFrame"] { overflow-x: auto !important; width: 100% !important; margin-bottom: 25px !important; }
}
</style>
""", unsafe_allow_html=True)

# ─── THE JAVASCRIPT DOM ESCAPE HATCH (REDIRECT BUG FIXED) ───
components.html("""
<script>
(function() {
    const doc = window.parent.document;
    
    // BUILD THE CYBERPUNK BOTTOM-LEFT MENU
    ...
    style.innerHTML = `
        #cyber-toggle-btn {
            position: fixed; bottom: 30px; left: 30px; width: 54px; height: 54px; border-radius: 12px;
            background: #0F172A; z-index: 99999999 !important; cursor: pointer; box-shadow: 0 0 20px rgba(0, 240, 255, 0.3);
            display: flex; align-items: center; justify-content: center; transition: transform 0.2s ease; overflow: hidden !important; 
        }
        #cyber-toggle-btn:active { transform: scale(0.92); }
        #cyber-toggle-btn::before {
            ...
            animation: border-trace 2s linear infinite; z-index: 0;
        }
        #cyber-toggle-btn .cyber-inner {
            position: absolute; inset: 3px; background: #0F172A; border-radius: 9px; z-index: 1;
            display: flex; align-items: center; justify-content: center; transition: background 0.3s;
        }
        #cyber-toggle-btn:hover .cyber-inner { background: #1E293B; }
        #cyber-toggle-btn svg { z-index: 2; }
        @keyframes border-trace { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } }
    `;
    doc.head.appendChild(style);

    // BULLETPROOF CLICK LOGIC: No fallbacks. Only precisely targets the actual toggle elements.
    btn.onclick = function() {
        try {
            ...
            // If the sidebar is open, the close button is visible.
...
# Taller default height (450px) to prevent layout squashing on mobile
def fancy_layout(h=450):
    return dict(
...
# ─── 5. SIDEBAR FILTERS ────────────────────────────────────────────────────────
with st.sidebar:
    st.markdown("""<h2 style='text-align: center; margin-top: 0; background: linear-gradient(90deg, #00F0FF, #FF2D7E); -webkit-background-clip: text; -webkit-text-fill-color: transparent;'>Global Filters</h2>""", unsafe_allow_html=True)
...
with col1:
    if not sk_filt.empty:
        ...
        L = fancy_layout(500); L['title'] = "Top 20 Skills Demanded"; L['xaxis_title'] = "Number of Mentions in Job Postings"
        fig.update_layout(**L)
        # CRITICAL FIX: theme=None forces Streamlit to respect our custom layout
    ...
    if not sk_filt.empty:
        ...
        for skill, color in zip(top8, COLORS):
            ...
            fig2.add_trace(go.Scatter(
                x=d["week"], y=d["mention_count"], name=skill, line=dict(color=color, width=3), mode='lines+markers',
                hovertemplate=f'<b>{skill}</b><br>%{{y}} mentions<extra></extra>'
            ))
        L2 = fancy_layout(500); L2['title'] = "Weekly Velocity — Top 8 Skills"; L2['legend'] = dict(orientation='h', y=-0.2)
...
if not sk_filt.empty:
    ...
    if not bi.empty:
        ...
        with ca:
            ...
            L3 = fancy_layout(450); L3['legend'] = dict(orientation='h', y=1.2); L3['xaxis'].update(tickangle=-45, nticks=10); L3['yaxis_title'] = "Weekly Mentions"
...
with col3:
    if not city_df.empty:
        ...
        fig4 = go.Figure(go.Bar(x=ct["city"], y=ct["job_count"], marker=dict(color=ct["job_count"], colorscale='Tealgrn')))
        L4 = fancy_layout(450); L4['title'] = "Job Volume by Hub"; L4['xaxis'].update(tickangle=-45)
...
with col4:
    if not role_df.empty:
        ...
        fig7 = go.Figure(go.Pie(
            ...
            marker=dict(colors=COLORS, line=dict(color='#05050A', width=3)), textinfo='label+percent', textposition='outside'
        ))
        L7 = fancy_layout(450); L7['title'] = "Role Category Split"; L7['showlegend'] = False
        fig7.update_layout(**L7)
        st.plotly_chart(fig7, use_container_width=True, config=PLOTLY_CONFIG, theme=None)
...
with col5:
    if not company_df.empty:
        ...
        fig6 = go.Figure(go.Bar(x=co["job_count"], y=co["company"], orientation='h', marker=dict(color=bar_colors), hovertemplate='<b>%{y}</b><br>%{x} openings<extra></extra>'))
        L6 = fancy_layout(500); L6['title'] = "Top 15 Hiring Companies"; L6['xaxis_title'] = "Total Active Job Openings" 
...
with col6:
    if not filt.empty:
        ...
        L8 = fancy_layout(500); L8['title'] = "Experience Distribution"
        L8['xaxis'].update(title="Minimum Years of Experience", dtick=1, showline=True, linewidth=1, linecolor='rgba(255,255,255,0.2)')
        L8['yaxis'].update(title="Number of Roles Available", showline=True, linewidth=1, linecolor='rgba(255,255,255,0.2)')
        fig8.update_layout(**L8)
        st.plotly_chart(fig8, use_container_width=True, config=PLOTLY_CONFIG, theme=None)
...
with col7:
    if not filt.empty and 'skills_extracted' in filt.columns:
        ...
        fig_hm = go.Figure(data=go.Heatmap(
            ...
            colorscale=[[0, '#0F172A'], [0.5, '#3B82F6'], [1, '#00F0FF']], hoverongaps=False,
            hovertemplate='<b>Role:</b> %{y}<br><b>Skill:</b> %{x}<br><b>Mentions:</b> %{z}<extra></extra>'
        ))
        L_hm = fancy_layout(500); L_hm['title'] = "Skill Dependency Matrix by Role"
        ...
        st.plotly_chart(fig_hm, use_container_width=True, config=PLOTLY_CONFIG, theme=None)
...
with col8:
    if not filt.empty and 'salary_min_lpa' in filt.columns:
        ...
        for idx, role in enumerate(top_roles):
            ...
            fig_box.add_trace(go.Box(y=role_data, name=role, marker_color=COLORS[idx % len(COLORS)], boxpoints='outliers', hovertemplate='<b>%{x}</b><br>Salary: ₹%{y} LPA<extra></extra>'))
            
        L_box = fancy_layout(500); L_box['title'] = "Realistic Salary Distribution (LPA)"
        L_box['yaxis_title'] = "Minimum Salary (LPA)"; L_box['xaxis'].update(tickangle=-45); L_box['showlegend'] = False 
        fig_box.update_layout(**L_box)
        st.plotly_chart(fig_box, use_container_width=True, config=PLOTLY_CONFIG, theme=None)

# ─── PLATFORM BREAKDOWN ────────────────────────────────────────────────────────
st.markdown('<div class="section-title" style="margin-top: 20px;">🌐 <span>Platform Breakdown</span></div>', unsafe_allow_html=True)
try:
    conn2 = sqlite3.connect(DB_PATH)
    plat = pd.read_sql("""SELECT CASE WHEN job_url LIKE '%linkedin%' THEN 'LinkedIn' WHEN job_url LIKE '%naukri%' THEN 'Naukri' WHEN job_url LIKE '%internshala%' THEN 'Internshala' ELSE 'Other' END as platform, COUNT(*) as count FROM jobs GROUP BY platform""", conn2)
    conn2.close()
    pc = st.columns(max(len(plat), 1))
    for i, (_, row) in enumerate(plat.iterrows()):
        with pc[i]:
            st.markdown(f"""<div class="plat-card" style="border-top: 2px solid #00F0FF;"><div style='font-size:0.75rem; letter-spacing:0.1em; color:var(--text-muted); text-transform:uppercase;'>{row['platform']}</div><div style='font-family:JetBrains Mono; font-size:1.8rem; font-weight:700; color:var(--cyan);'>{row['count']:,}</div><div style='font-size:0.65rem; color:var(--text-muted)'>JOBS SCRAPED</div></div>""", unsafe_allow_html=True)
...
if not filt.empty:
    ...
    if search:
        mask = (latest["job_title"].str.contains(search, case=False, na=False) | latest["company"].str.contains(search, case=False, na=False) | latest["skills_extracted"].str.contains(search, case=False, na=False))
        latest = latest[mask]
